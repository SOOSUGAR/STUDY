...
public class HomeController {
   ...
   public String ezen(Model model) {
      // Model 객체는 Request영역에서 데이터를 저장할 수 있는 객체
   ...
   public String test2Get(@RequestParam Map<String, Object> paramMap) {
	   System.out.println(paramMap.get("firstName") + " " + paramMap.get("lastName"));
	   
	   System.out.println(paramMap.get("job") + " " + paramMap.get("age"));
   ...
   // 파라미터로 넘어오는 데이터의 키값과 command객체의 멤버변수의 이름이 같으면, 그 멤버변수에 데이터의 값이 입력
   @PostMapping("test3.do")
   public String test3post(NameVO nameVO) {
	   System.out.println(nameVO.getFirstName() + " " + nameVO.getLastName() + " " + nameVO.getJob() + " " + nameVO.getAge());
...
//	   request.setAttribute("list", mapList);
	   ...
	   // 3. HttpSession 객체 사용
	   //	 HttpSession > HttpServletRequset
